				...
				<div class="row clearfix">
					...
					<div class="col-md-12 column">
						...
						<table class="table">
							...
							<tbody>
								<c:forEach items="${leaveListFromServer }" var="leave">
									<tr>
										<td>${leave.leaveId }</td>
										<td>${leave.empno }</td>
										...
										<td><a
											href="${pageContext.request.contextPath }/leave_delete.action?leaveId=${leave.leaveId}"
											onclick="return confirm('确认删除吗？')">删除</a></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
						请假ID： <input type="number" value="1" id="leaveId" /> <a
							href="${pageContext.request.contextPath }/leave_find_findById.action?leaveId="
							onclick="findByIdHandler()" id="findById">精确查询</a>
						<script>
							function findByIdHandler() {
								//1-获取input-leaveId
								var leaveId = document.querySelector("#leaveId").value;
								console.log(leaveId);
								//2-获取a节点，拼接href属性
								var aNode = document.querySelector("#findById");
								console.log(aNode.href);
								aNode.href = aNode.href + leaveId;
								console.log(aNode.href);
								//3-实现跳转
								return true;
							}
						...
						<div style="float: right;">
							<a
								href="${pageContext.request.contextPath}/router_saveLeave.action ">添加信息</a>
						</div>
					</div>
				</div>
			</div>

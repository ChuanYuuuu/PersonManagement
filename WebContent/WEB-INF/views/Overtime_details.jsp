				...
				<div class="row clearfix">
					...
					<div class="col-md-12 column">
						...
						<table class="table">
							...
							<tbody>
								<c:forEach items="${overtimeListFromServer }" var="overtime">
									<tr>
										<td>${overtime.overId }</td>
										<td>${overtime.empno }</td>
										<td>${overtime.overDate }</td>
										<td>${overtime.overReason }</td>
										<td><a
											href="${pageContext.request.contextPath }/overtime_delete.action?overId=${overtime.overId}"
											onclick="return confirm('确认删除吗？')">删除</a> <%-- <a
											href="${pageContext.request.contextPath }/overtime_find_findById.action?type=update&overId=${overtime.overId}">修改</a> --%>
										</td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
						部门ID： <input type="number" value="1" id="overId" /> <a
							href="${pageContext.request.contextPath }/overtime_find_findById.action?overId="
							onclick="findByIdHandler()" id="findById">精确查询</a>
						<script>
							function findByIdHandler() {
								//1-获取input-overId
								var overId = document.querySelector("#overId").value;
								console.log(overId);
								//2-获取a节点，拼接href属性
								var aNode = document.querySelector("#findById");
								console.log(aNode.href);
								aNode.href = aNode.href + overId;
								console.log(aNode.href);
								//3-实现跳转
								return true;
							}
						...
						<div style="float: right;">
							<a
								href="${pageContext.request.contextPath}/router_saveOvertime.action ">添加加班项</a>
						</div>
					</div>
				</div>
			</div>
